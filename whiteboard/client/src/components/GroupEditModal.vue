...
export default {
  ...
  computed: {
    group() {
      return Object.assign({}, this.$store.state.currentGroup);
    },
    show() {
      return this.$store.state.showGroupEdit;
    },
  },
  watch: {
    show(newVal) {
      if (newVal) {
        ...
        $(this.$el).one('hidden.bs.modal', () => {
          component.$store.commit('setCurrentGroup', {});
          component.$store.commit('toggleGroupEdit');
        });
      }
    },
  },
  methods: {
    ...
    checkSize() {
      this.sizeError = this.group.size === '';

      if (!this.sizeError) {
        const size = parseInt(this.group.size, 10);
    ...
    updateGroup() {
      ...
      const data = Object.assign(
        {}, this.group,
        { max_members: parseInt(this.group.max_members, 10) });
